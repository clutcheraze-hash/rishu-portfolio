...
## ⚠️ Important: This is a single-page portfolio

The `src/routes/index.tsx` file is the ENTIRE portfolio. All sections (Hero, About, Experience, Skills, Projects, Achievements, Contact) are React components in that one file. Other routes redirect to `/`.

---

## Architecture — Kai Tanaka Premium Portfolio
...
src/
  routes/
    __root.tsx        — HTML shell, global meta, font preloads, grain overlay
    index.tsx         — Complete portfolio (all 7 sections as components + data)
    contact.tsx       — Redirects to /
    resume.tsx        — Redirects to /
    projects.tsx      — Redirects to /
  styles.css          — Full design system (CSS custom props, all component styles)
  components/ui/      — Unused Shadcn UI primitives

content/              — Content Collections markdown (unused by portfolio)
public/
  headshot-on-white.jpg  — Portrait image used in About section
```

### Architecture Decisions

**Single-file portfolio**: All content lives in `index.tsx`. No CMS, no dynamic routes, no API calls — just a cinematic single-page experience.

**CSS animation system**: `IntersectionObserver` toggles `.sr → .visible` classes. CSS transitions handle all scroll reveals. No JS animation framework in the critical path.

**Lenis (lazy)**: Smooth scroll is dynamically imported *after* the loading screen, avoiding blocking initial render.

**Custom cursor**: `requestAnimationFrame` loop with linear interpolation (`lerp(0.12)`) for the trailing ring. Dot snaps instantly; ring follows with physics-like lag.

**Particle system**: 60 canvas particles with mouse attraction within 200px radius, using crimson/silver/off-white colors.

### Editing Content

Update the constant arrays at the top of `src/routes/index.tsx`:
- `EXPERIENCES` — Work history
- `SKILLS` — Skill bars (0–1 float = percentage)
- `PROJECTS` — Venture cards
- `ACHIEVEMENTS` — Animated counters

### Design Tokens

All in `src/styles.css` as CSS custom properties: `--black`, `--crimson`, `--off-white`, `--font-display`, etc. Do not use Tailwind utilities for design decisions — use these variables.

---

Original scaffold context below:
